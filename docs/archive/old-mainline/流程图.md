...
校验这条主链时，参考版行为是主合同；已有推送记录只作为回归线索和历史意图证据。
...
flowchart TD
    ...
    H --> I[element_process<br/>已有 cross 优先<br/>无 cross 再检查 ring]
...
- 新 cross 优先于新 ring；这对齐 RT1064 `main.c` 先 `check_cross()`、
  cross 存在时清 `circle_type` 的调度。
- ring 只在没有 cross 时推进或检测入口。
...
环岛入口按 RT1064 `circle.c::check_circle()` 形状：单侧 L 角点 +
对侧长直线直接进入 BEGIN。旧的 Unity CD.cpp `AIB/BW/pending/ring_times`
入口已经归档到 `docs/reference/unity_CD_old.cpp`，不再是当前主线合同。
