...
int have_left_line = 0, have_right_line = 0;         /* 环岛/诊断有线计数 */
...
int circle_cal_log_enabled(void)
{
    ...
    if(!warned_invalid)
    {
        printf("环境警告: FRONT_CAR_CIRCLE_CAL_LOG='%s' 不是有效开关，只接受 0/1/false/true\n", val);
        warned_invalid = 1;
    }
    return 0;
}
...
static void log_circle_state(enum circle_type_e from, enum circle_type_e to, const char *reason)
{
    if(circle_cal_log_enabled())
    {
        printf("ATGCircleState: from=%s to=%s reason=%s heading_deg10=%d dist=%d begin_dist=%lld begin_last=%lld\n",
               ...
               total_distence,
               (long long)begin_dist,
               (long long)last_begin_dist);
    }
}
 ...
 * 与 vision_step.c 的 reset_circle_to_none() 保持一致。
 * 调用时机：OUT 阶段连续检测到直道后。 */
...
/* BEGIN 阶段异常撤回（假入口、口部丢失过晚等），不记录为完成圆环。
 * 与 finish_circle_exit 的区别：这不是真正的圆环。
...
static void log_circle_abc(int side, const char *phase, int mouth_ready)
{
    ...
    printf("ATGCircleABC: side=%c phase=%s ref=%d(%s) rpts=%d/%d lpt=%d@%d/%d@%d "
           ...
           mouth_ready, circle_heading_deg10(), (long long)begin_dist);
}
...
static void update_begin_loss(int side)
{
    ...
    if(lost)
    {
        ...
        {
            circle_loss_start_begin_dist[side] = begin_dist;
        }
