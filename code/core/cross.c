...
static float conf3, conf4;                            /* 远端 L 角点的当前角度置信度 */
float conf3_max,conf4_max;                            /* 远端 L 角点的历史最大置信度 */
...
/* 远线搜索起始坐标 */
int far_x1 = 32, far_x2 = 118, far_y1;               /* 左/右远线搜索起点（原图坐标） */
static int far_y2;
...
static void reset_cross_state(void);
static void cross_farline(void);
...
void run_cross(void)
{
    ...
    if (cross_type == CROSS_BEGIN) {
        ...
        aim_distance = 0.4; /* 十字 BEGIN 阶段使用近前瞻 */
