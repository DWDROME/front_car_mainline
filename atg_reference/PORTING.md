...
| Joshua-style circle entry first identifies same-side entry geometry, then uses line repair to make ordinary line following enter the island. | The local port had A (`Lpt0/Lpt1`) but no B/C geometry, so old entry progression came from `mouth_loss + begin_dist` and the old fixed line. | `circle.c` now treats A as `Lpt0/Lpt1`, validates the entry with inner-hit plus B on the opposite arc after skipping the near-side line edge (left circle starts near `seed_x+20` and scans right; right circle starts near `seed_x-20` and scans left), keeps the state in `BEGIN`, and sets `circle_ref_mode=IN_C` only after C is stable. `reference_step.c` publishes `circle_in_c_left/right` from the C-anchored repaired boundary using the existing half-line helper. | This makes entry timing and entry path depend on current-frame A/B/C geometry instead of a loss/distance window, while staying inside the existing ATG `rpts`/IPM/selected-line pipeline. | If B/C are absent, `BEGIN` does not silently fall back to the old fixed line; selected line failure is visible through `ATGCircleABCDiag`, source ids `15/16`, and the existing stall reset. |
