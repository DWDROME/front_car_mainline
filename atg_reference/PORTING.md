...
| 层 | 目录 | 规则 |
| --- | --- | --- |
| ATG 原始参考 | `atg_reference/Project/CODE/`、`atg_reference/port/` | 只读参考来源，不参与当前主目标构建。 |
| ATG 主循环证据 | `atg_reference/Project/USER/` | 仅供只读参考，用于确认执行顺序、选线逻辑与元素序列。 |
| 当前视觉核心 | `code/core/` | 当前构建使用的 ATG-derived 搜线、元素和补线算法。 |
| 当前视觉主流程 | `code/app/vision_step.c` | 按一帧视觉顺序调度 core 算法。 |
| 当前 ATG 胶水 | `code/app/headfile.h` | 主线可见的 ATG 全局总头，包含类型别名、宏、LCD 空实现和全局声明；`code/port/` 不再承载当前主线文件。 |
| 当前运行层 | `code/app/` | `live()` / 离线帧处理直接调用 `vision_step()`，再直接读取 `rptsn`/`Guide`/元素状态写入 `runtime_t` 和控制输入。 |
| 当前诊断层 | `code/report/` | `vision_view.cpp` 给 report/replay/assistant 提供快照、点列和坐标辅助。 |
...
5. 视觉主链下放到 `code/core` + `code/app/vision_step.c`；旧的 `autop_reference/` 目录树与桥接从本分支移除。
6. 使用 `git diff --check`、`bash code/test.sh --host` 及定向离线/分析帧进行验证。

## 当前阶段

本分支当前编译的 ATG-derived 循迹与元素主线位于 `code/core` 和 `code/app/vision_step.c`；
`atg_reference/` 仅作为原始参考保留：

| 构建中活跃 | 状态 |
| --- | --- |
| `code/core/shy_Image.c` | 活跃，已迁入 core |
| `code/core/imgproc.c` | 活跃，已迁入 core |
| `code/core/utils.c` | 活跃，已迁入 core |
| `code/core/cross.c` | 活跃，已迁入 core |
| `code/core/Half_check.c` | 活跃，已迁入 core |
| `code/core/circle.c` | 活跃，已迁入 core |
| `code/core/Patching_Line.c` | 活跃，已迁入 core |
| `code/core/round.c` | 活跃，已迁入 core |
| `code/core/yroad.c` | 活跃，已迁入 core |
| `code/core/Ramp.c` | 活跃，已迁入 core |
| `code/core/road.c` | 活跃，已迁入 core |
| `code/app/vision_step.c` | 活跃，一帧视觉主线 |
| `code/core/vision_globals.c` | 活跃，视觉全局变量存放 |
| `code/report/vision_view.cpp` | 活跃，诊断快照和点列视图 |
...
`code/app/vision_step.c` 现为一帧视觉主线：它按 ATG 顺序调用搜线、角点、
元素处理、选线和归一化。`code/app/runners.cpp` 在调用 `vision_step()` 后，
直接把当前帧 `rptsn` 中线和导引误差写入 `rt->vision`。它不引入 ATG 舵机 PID 或电机输出；当前控制仍为
`guide_error -> yaw_cmd -> 差速电机占空比`。

TC264 车库动作、电机/舵机 PID、UART 广播、TFT 显示、Flash、按键及传感器
驱动仍在构建之外。环岛、坡道、Y 字路和车库逻辑保留在复制的 ATG 源码中
以便后续复用，但 `vision_step.c` 在当前比赛配置中不激活这些状态。

## 修改理由

| 参考版原本 | 当前代码差异 | 做了什么修改 | 原因 | 不引入兜底错误的证据 |
| --- | --- | --- | --- | --- |
| `Project/USER/Cpu0_Main.c` 依次执行 `image_handle()`、`find_corners()`、元素检测、元素运行及选线归一化。 | 此前 ATG 分支仅运行基础边线搜索并复制 `rptsc0/rptsc1`，已复制的元素文件未编译也未调用。 | `code/app/vision_step.c` 现在调用 ATG 的十字/半十字/圆环序列及选线逻辑。 | 算法主线必须依循 ATG 状态机的拥有者，而非旧的本地运行时。 | 几何缺失时仍返回无 `rptsn`；不复用旧帧中线。 |
...
| ATG `circle.c` 配合 `Patching_Line.c` 构建圆环各阶段的拼接中线。 | 圆环状态不再翻译到本地环诊断。 | `vision_step.c` 为 ATG 圆环 `BEGIN`/`RUNNING`/`OUT` 状态构建拼接中线；上报/辅助工具通过 `vision_view.cpp` 读取 `circle_type`、`circle_ref_mode` 及 `Splicing_*` 证据。 | 圆环选线行为属于已购入的 ATG 改进路径的一部分。 | 不产生合成兜底线；仅当 ATG 状态与远线证据使该分支活跃时才拼接。 |
| ATG `Cpu0_Main.c` 还包含环岛、坡道、Y 字路和车库相关分支。 | 当前比赛不需要这些元素，且部分触发条件依赖传感器或场景约定，未在本车上验证。 | 保留复制的源码，但通过 `ATG_ENABLE_*` 进行门控：仅启用十字/半十字和圆环；环岛/坡道/Y 字路/车库保持在 `NONE`。 | 这样保留 ATG 代码可用，同时避免未启用的元素抢占状态机。 | 禁用状态在 `vision_step.c` 中每帧显式重置；不引入旧帧线或假元素状态。 |
...
| Joshua 风格的圆环入口首先识别同侧入口几何，然后使用线段修复使普通循迹进入环岛。 | 本地移植有 A（`Lpt0/Lpt1`）但无 B/C 几何，因此旧的入口推进来自 `mouth_loss + begin_dist` 和旧固定线。 | `circle.c` 现在将 A 视为 `Lpt0/Lpt1`，用 inner-hit 验证入口，并在跳过近侧边线边缘后扫描另一弧上的 B（左环扫 `seed_x+20..seed_x+45`；右环扫 `seed_x-20..seed_x-45`），状态保持在 `BEGIN`，仅 C 稳定后才设 `circle_ref_mode=IN_C`。`vision_step.c` 使用已有的半线辅助函数从 C 点锚定的修复边界发布 `circle_in_c_left/right`。 | 这使入口时机与入口路径依赖当前帧 A/B/C 几何而非丢线/距离窗口，同时保持在既有 ATG `rpts`/IPM/选线管线之内。 | 若 B/C 缺失，`BEGIN` 不会静默回退到旧固定线；选线失败通过 `ATGCircleABCDiag`、源 id `15/16` 及已有 stall 复位可见。 |
...
| ATG 圆环可依赖持续运动来达到后续视觉或距离出口。 | 本差速车在 `line_found=0` 时停止；距离兜底移除后，未能推进的圆环状态必须可见地失败，而非被另一窗口掩盖。 | `vision_step.c` 保留主线 stall 出口：在"圆环活跃但无选线"连续 30 帧后，以显式 `ATGCircleStall` 日志复位圆环状态。`build_circle_spliced_lines()` 每帧仍清除 `Splicing_*_center_num`。 | stall 复位现为唯一的非陀螺/非视觉逃生口，与当前清理决策一致。 | 不伪造线、不复用旧帧：车保持停止直到真实当前帧证据产生线；复位仅释放物理前提已不存在的状态，并以显式日志而非静默失败。 |
