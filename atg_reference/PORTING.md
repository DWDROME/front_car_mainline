...
| Joshua-style circle entry first identifies same-side entry geometry, then uses line repair to make ordinary line following enter the island. | The local port had A (`Lpt0/Lpt1`) but no B/C geometry, so `BEGIN -> IN` still came from `mouth_loss + begin_dist` and `IN` unconditionally used the old fixed line. | `circle.c` now treats A as `Lpt0/Lpt1`, finds B as the same-side inward x-extreme after A, switches to IN only when B is stable and reaches `raw_y >= 58`, then finds C as a deeper V point. `reference_step.c` publishes `circle_in_c_left/right` only after C is stable and builds the center line from the C-anchored repaired boundary using the existing half-line helper. | This makes entry timing and entry path depend on current-frame A/B/C geometry instead of a loss/distance window, while staying inside the existing ATG `rpts`/IPM/selected-line pipeline. | If B/C are absent, IN does not silently fall back to the old fixed line; selected line failure is visible through `ATGCircleABCDiag`, source ids `15/16`, and the existing stall reset. |
