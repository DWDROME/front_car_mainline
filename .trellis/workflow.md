...
## ⚠️ MUST READ: Project Context (读完再动手)

本项目是 **ATG2022 舵机车算法移植到龙芯 LS2K0300 差速车**。所有问题都来自舵机→差速车的"语言不匹配"。

**开始任何 tracking 相关工作前，必须先读：**

1. **架构速查 + 三参考源** → `.trellis/spec/tracking/critical-audit-guide.md`
2. **参考摘要** → `.reference/joshua-xu-smartcar/SUMMARY.md`（含 ATG/Joshua.Xu/ittuann 三源关键信息）
3. **详细架构** → `docs/01_全局地图.md`、`docs/02_当前主链与参考版对比.md`
4. **调参和验证** → `docs/03_调参与验证手册.md`
5. **当前问题** → `docs/04_当前问题.md`

**硬边界（别碰）：**
- `pure_angle` 是参考版舵机角，**不进控制环**
- IPM 生产真相源只有 `shy_Image.c::rot/inv_rot`
- 不用上一帧中线/边界伪造当前帧几何
...
**Critical tracking add-on**: for cross/ring/mainline/boundary/imgproc/control tracking work, treat `.trellis/spec/tracking/critical-audit-guide.md` and `.trellis/spec/quality/embedded-tracking-guidelines.md` as mandatory context. Prefer `fast_context_search` for unclear local discovery, but if the MCP is slow to appear retry before downgrading to `rg` + direct reads. Use `smart-search-cli` for external/current/documentation/URL facts. Run or account for four fixed read-only audit roles — reference contract, current implementation, git regression, and Trellis/spec/check gate — and keep C/C++ edits simple, direct, and free of stale-geometry fallbacks.
